...
struct LUDecompositionSolverView: View {
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 16) {
                Text("LU Decomposition Solver").font(.largeTitle.bold())
                Text("Partial pivoting is used. Matrix and vector values are dimensionless.").foregroundStyle(.secondary)
    ...
    private func calculate() {
        do { let result = try LUDecompositionSolverEngine().solve(.init(matrix: parseMatrix(matrixText), constants: parseVector(vectorText))); resultText = result.solution.map { String(format: "%.8g", $0) }.joined(separator: ", "); errorText = nil }
