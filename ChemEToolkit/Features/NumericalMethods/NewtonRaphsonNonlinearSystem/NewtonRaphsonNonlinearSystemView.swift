...
struct NewtonRaphsonNonlinearSystemView: View {
    ...
    var body: some View { ScrollView { VStack(alignment: .leading, spacing: 16) {
        Text("Newton–Raphson Nonlinear System").font(.largeTitle.bold())
    ...
    private func calculate() { do { let r = try NewtonRaphsonNonlinearSystemEngine().solve(.init(system: .circleAndLine, initialGuess: [Double(x) ?? .nan, Double(y) ?? .nan])); output = r.solution.map { String(format: "%.8g", $0) }.joined(separator: ", "); errorText = nil } catch { output = nil; errorText = error.localizedDescription } }
}
